...
input_dir='inputs/distshift'
output_root_dir='outputs/distshift_results'
req_dist='zipf'
...
do
  dataset=${workload}'-80R-50K-'${req_dist}
  ...
  mv ${output_dir}/${dataset}-weights.txt ${output_dir}/ds_${dataset}_${input_dim}D${hidden_dim}H${layer}L_weights.txt
  cp ${output_dir}/ds_${dataset}_${input_dim}D${hidden_dim}H${layer}L_weights.txt ${output_dir}/ds_${workload}-20R-50K-${req_dist}_${input_dim}D${hidden_dim}H${layer}L_weights.txt
done